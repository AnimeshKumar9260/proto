...
export default function PvtLtdPlanningMob() {
  ...
  return (
    <div className="py-12 md:hidden px-4 md:px-10">
      ...
      <div className="flex justify-between mb-8">
        {curCard > 0 && (
          <button
            onClick={() => setCurCard(curCard - 1)}
            className="bg-orange-200 py-2 px-4 rounded"
          >
            &larr; {planData[curCard - 1].type}
          </button>
        )}
        <div></div>
        {curCard < planData.length - 1 && (
          <button
            onClick={() => setCurCard(curCard + 1)}
            className="bg-orange-200 py-2 px-4 rounded"
          >
            {planData[curCard + 1].type} &rarr;
          </button>
        )}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
        <PvtCard data={planData[curCard]} />
      </div>
    </div>
  );
}
